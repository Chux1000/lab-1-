...
products.forEach((product, index) => {
  ...
  item.className = "item";
  item.innerHTML = `
    <img src="/Miva work/miva collection/${product.image}" alt="${product.name}">
    <h3>${product.name}</h3>
    <p>$${product.price}</p>
...
function openModal(index) {
  ...
  document.getElementById(
    "modalImg"
  ).src = `/Miva work/miva collection/${product.image}`;
...
for (let i = 2; i <= 20; i++) {
  ...
  item.className = 'item';
  item.innerHTML = `
    <img src="/Miva work/miva collection/pictures/item${i}.png" alt="Item ${i}">
    <h3>Item ${i}</h3>
    <p>$${(i * 10).toFixed(2)}</p>
